...
# import matplotlib
# try:
#     # Try to use the default interactive backend
#     matplotlib.use('TkAgg')  # or 'TkAgg', etc.
# except ImportError:
#     # Fallback to Agg if the GUI backend is not available
#     matplotlib.use('Agg')
...
# file_path = '/mnt/f/Research/new_roary/analysis/out.emapper.annotations'
# output_path = '/mnt/f/Research/new_roary/analysis/'
...
#         go_id_counts.to_csv(output_path,'go_id_counts.csv', header=True)
...
#         plt.show()
#         plt.savefig(file_path,'output_plot.png') 
...
try:
    ...
    if 'GOs' in annotation_df.columns:
        ...
        plt.title('Top 20 GO IDs by Count')
        plt.gca().invert_yaxis()  # Highest counts at the top
        plt.savefig(output_path + 'output_plot.png')  # Save the plot
